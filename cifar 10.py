...
parser.add_argument('--dataset', default='cifar10', help='dataset name') # cifar100
...
parser.add_argument('--noise_ratio', default=0.5, type=float, help='artifical noise ratio (default: 0.5)')  # 0.5
...
def var_estimate(logits):
    """Compute entropy to estimate noise level for each sample in the batch."""
    probabilities = F.softmax(logits, dim=1)
    return torch.var(probabilities, dim=1)

def puzzle_mix(input_a, input_b, level, n_splits=4):
    ...
    mix_flag = torch.rand(n_splits, n_splits) > level
...
def train(labeled_trainloader, modified_label, all_trainloader, encoder, classifier, proj_head, pred_head, optimizer,
    ...
    for [inputs_x1, inputs_x2], labels_x, _, index in labeled_train_iter:
        ...
        with torch.no_grad():
            initial_logits1 = classifier(encoder(inputs_x1))

        clean_level1 = var_estimate(initial_logits1)  # .mean().item()  # Mean entropy across the batch

        with torch.no_grad():
            initial_logits2 = classifier(encoder(inputs_x2))

        clean_level2 = var_estimate(initial_logits2)

        level_1all = torch.sum(clean_level1)
        level_2all = torch.sum(clean_level2)

        level1 = level_1all / (level_1all + level_2all)
        level1 = level1.cpu().item()
        ...
        mixed_input = puzzle_mix(input_a, input_b, level1)

        mixed_target = level1 * target_a + (1 - level1) * target_b
...
def evaluate(dataloader, encoder, classifier, args, noisy_label, clean_label, i, stat_logs):
    ...
    with torch.no_grad():
        ...
        # flattened_list = [item for sublist in feature_arrbank for item in sublist]

        concatenated_array = np.concatenate(feature_arrbank, axis=0)

        # feature_bank = torch.from_numpy(np.array(flattened_list))
        ...
        variance = torch.var(prediction_cls, dim=1)
        ...
        combined_score = norm_variance - norm_entropy 

        top_element = int(combined_score.numel() * (i + 1) / args.epochs)
        top_values, conf_id = torch.topk(combined_score, top_element)
...
def main():
    ...
    # criterion_unsup = torch.nn.CrossEntropyLoss().cuda(0)
